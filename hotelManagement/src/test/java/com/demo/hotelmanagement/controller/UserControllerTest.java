 ...
 * 
 * 
 * import org.hamcrest.Matchers; import org.junit.jupiter.api.BeforeEach; import
 ...
 * 
 * 
 * import com.demo.hotelmanagement.dto.LoginDto; import
 * com.demo.hotelmanagement.dto.UserDto; import
 * com.demo.hotelmanagement.service.UserService; import
 ...
 * 
 * 
 * @BeforeEach public void setUp() { objectMapper=new ObjectMapper();
 ...
 * 
 * 
 * @Test public void registerTest() throws Exception{
 * mockMvc.perform(post("/users").contentType(MediaType.APPLICATION_JSON_VALUE)
 * .content(objectMapper.writeValueAsString(userDto)))
 * .andExpect(status().isOk()) .andExpect(jsonPath("$",
 * Matchers.is("user added successfully")));
 * 
 * }
 * 
 * @Test public void loginTest() throws Exception { //given
 * when(userService.authenticateUser("manasa", "1234")).thenReturn(true);
 * 
 * //when and then mockMvc.perform(post("/users/login").contentType(MediaType.
 * APPLICATION_JSON_VALUE) .content(objectMapper.writeValueAsString(loginDto)))
 * .andExpect(status().isOk()) .andExpect(jsonPath("$",
 * Matchers.is("logged in successfully")));
 * 
 * verify(userService).authenticateUser("manasa", "1234"); }
 * 
 * @Test public void loginTest1() throws Exception { //given
 * when(userService.authenticateUser("manasa", "1234")).thenReturn(false);
 * 
 * //when and then mockMvc.perform(post("/users/login").contentType(MediaType.
 * APPLICATION_JSON_VALUE) .content(objectMapper.writeValueAsString(loginDto)))
 * .andExpect(status().isOk()) .andExpect(jsonPath("$",
 * Matchers.is("Credentials are incorrect")));
 * 
 * verify(userService).authenticateUser("manasa" ,"1234"); }
 * 
 * 
